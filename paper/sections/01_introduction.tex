...
arms at matched LLM-token, retrieval, and memory budgets, with a single
\emph{preregistered} primary endpoint, a ground-truth-blind random-pruning
negative control, an oracle prune ceiling, a long-context baseline (the key
validity threat), and an A/A control that fixes the noise floor any claimed
effect must clear. The verdict is recorded whichever way the data fall, and a
...
\begin{itemize}
  ...
  cycle.
  \item \textbf{A protocol.} A preregistered, matched-budget experimental design
  ...
  mechanism-demonstration regime: a confirmed matched-budget improvement of the
  full dream cycle over the no-sleep baseline on the primary endpoint
  ($\Delta=\SWprimaryDiff$); a replay-targeting effect that exceeds a
  targeted-memory-reactivation benchmark; \emph{and}, reported with equal weight,
  the finding that a long-context baseline dominates on raw accuracy at maximum
  cost with \emph{no} cost-adjusted crossover in the swept stream lengths ---
  mapping the cost regime in which external consolidation is, and is not, worth
  paying for.
\end{itemize}
