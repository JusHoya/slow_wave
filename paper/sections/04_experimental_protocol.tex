...
retrieval, and memory budgets (the precondition \emph{as preregistered}; the
realised grid did not meet it, so the verdict is reported on the budget-agnostic
endpoint and the accuracy-vs-compute Pareto frontier below), the full
four-operator dream cycle
...
count as confirmed. In the realised grid the matched-budget controller finds
\texttt{full\_dream} out of tolerance on tokens and memory vectors --- it spends
more of both than \texttt{no\_sleep} (Table~\ref{tab:arms}) --- so the committed
\texttt{budget\_matched} verdict is false in every regime; per the preregistered
fallback below, the primary and secondary contrasts are therefore reported on the
accuracy-vs-compute Pareto frontier, as cost-effectiveness results rather than
matched-budget free lunches.

\paragraph{Control battery.} \SWnArms{} arms share one harness under a
matched-budget controller: \texttt{no\_sleep} (wake-only baseline), \texttt{replay\_only},
...
(two identical configs differing only in seed, whose cross-seed mean-ACC gap is
the empirically measured noise floor any claimed effect must exceed). A
matched-budget controller equalises LLM tokens, retrieval calls, and final
memory-vector count across arms within tolerance and records the actuals; where
matching is infeasible within tolerance --- as it is for \texttt{full\_dream} in
this grid --- we report an accuracy-vs-compute Pareto frontier (maximise ACC,
minimise tokens) instead of a single matched point. On that frontier
\texttt{reflection} is a fully \emph{deployable} comparator (unlike the
\texttt{oracle}/\texttt{long\_context} ceilings), so the full cycle's position
relative to it is a first-class comparison, reported alongside the long-context
threat in Section~\ref{sec:results}, not a sanity bound.
...
\citep{chaudhry2018riemannian}. Forward transfer is structurally zero on this
task-incremental stream --- independent per-task fact sets leave nothing to
transfer forward before a task is first seen --- so we report it for completeness,
not as a discriminating metric (Section~\ref{sec:results}). The bench's
distinctive measurement is
